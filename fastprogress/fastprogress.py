    ...
    try:
        ...
        if shell == 'ZMQInteractiveShell':
            ...
            if program == 'spyder':
                ...
                WRITER_FN('fastprogress: spyder is not notebook')
                return False, True # Spyder
            if program in ('ipython', 'jupyter'):
                if 'JPY_PARENT_PID' in env:
                    WRITER_FN('fastprogress: notebook is notebook')
                    return True, False # Jupyter notebook
            WRITER_FN('fastprogress: qtconsole is not notebook')
            return False, True  # Jupyter qtconsole
        elif shell == 'TerminalInteractiveShell':
            return False, False  # Terminal running IPython
        else:
            return False, False  # Other type (?)
    except NameError:
        return False, False     # Probably standard Python interpreter

IN_NOTEBOOK, IN_QTCONSOLE = isnotebook()
if IN_NOTEBOOK:
    try:
...
class ProgressBar():
    ...
    def __init__(self, gen,
                 total=None, display=True, leave=True, parent=None, auto_update=True):
        self._gen = gen
...
class NBMasterBar(MasterBar):
    names = ['train', 'valid']
    def __init__(self, gen,
                 total=None, hide_graph=False, order=None,
                 clean_on_interrupt=False, total_time=False):
        super().__init__(gen, NBProgressBar, total)
        self.report = []
        self.clean_on_interrupt, self.total_time = clean_on_interrupt, total_time
...
class ConsoleProgressBar(ProgressBar):
    ...
    def __init__(self, gen,
                 total=None, display=True, leave=True, parent=None,
                 auto_update=True, txt_len=60):
    ...
    def on_iter_end(self):
        if not self.leave and printing():
            WRITER_FN(f'\r{self.prefix}' + ' ' * (self.max_len - len(f'\r{self.prefix}')),
                  end = '\r')

    def on_update(self, val, text):
        if self.display:
            self.length = min(self.cols - len(text) - len(self.prefix) - 4, self.length)
...
class TqdmProgressBar(ProgressBar):
    def __init__(self, gen,
                 total=None, display=True, leave=True, parent=None, auto_update=True, txt_len=60):
        cls_pb = tqdm.tqdm_notebook if IN_NOTEBOOK else tqdm.tqdm
        self.pb = cls_pb(gen, total=total, disable=(not display), leave=leave)
        super().__init__(gen, total, display, leave, parent, auto_update)
    ...
    def on_update(self, val, text):
        self.pb.set_description(text)
        self.pb.update(val - self.true_last_v)

    def update(self, val):
        if val == 0:
            self.true_last_v = 0
        else:
            self.true_last_v = self.last_v
        super().update(val)

    def write(self, line):
        self.pb.write(line)

class TqdmMasterBar(MasterBar):
    def __init__(self, gen,
                 total=None, hide_graph=False, order=None,
                 clean_on_interrupt=False, total_time=False):
        super().__init__(gen, TqdmProgressBar, total)
        self.total_time = total_time
    ...
    def write(self, line,
        ...
        if table:
            ...
            self.first_bar.write(text)
        else:
            self.first_bar.write(line)
        if self.total_time:
            total_time = format_time(time() - self.start_t)
            self.first_bar.write(f'Total time: {total_time}')
...
def printing():
    return False if NO_BAR else (stdout.isatty() or IN_NOTEBOOK or IN_QTCONSOLE)

if IN_NOTEBOOK:
    master_bar, progress_bar = NBMasterBar, NBProgressBar
    WRITER_FN('fastprogress: using notebook backend')
elif IN_QTCONSOLE or not HAS_TQDM:
    master_bar, progress_bar = ConsoleMasterBar, ConsoleProgressBar
    WRITER_FN('fastprogress: using console backend')
else:
    master_bar, progress_bar = TqdmMasterBar, TqdmProgressBar
    WRITER_FN('fastprogress: using tqdm backend')
